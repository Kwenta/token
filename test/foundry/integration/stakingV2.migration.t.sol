...
contract StakingV2MigrationTests is StakingTestHelpers {
    ...
    function test_Migrate_Then_Move_Funds_From_V1_To_V2_And_Generate_New_Rewards(
        ...
        uint256 user2Account = createStakingAccount(user2);
        uint256 user3Account = createStakingAccount(user3);

        // Migrate non-escrow stake from StakingRewardsV1 to StakingRewardsV2
        exitStakingV1(user1);
        stakeFundsV2(user1Account, user1NonEscrowedStakeV1);
        exitStakingV1(user2);
        stakeFundsV2(user2Account, user2NonEscrowedStakeV1);
        exitStakingV1(user3);
        stakeFundsV2(user3Account, user3NonEscrowedStakeV1);

        uint256 user1NonEscrowedStakeV2 =
            stakingRewardsV2.nonEscrowedBalanceOf(user1Account);
        uint256 user2NonEscrowedStakeV2 =
            stakingRewardsV2.nonEscrowedBalanceOf(user2Account);
        uint256 user3NonEscrowedStakeV2 =
            stakingRewardsV2.nonEscrowedBalanceOf(user3Account);
