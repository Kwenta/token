...
contract TokenDistributorTest is StakingSetup {
    ...
    function testFuzzTimeOffset(uint amount, uint staking1, uint staking2, uint staking3, uint time) public {
        TokenDistributor tokenDistributorOffset = new TokenDistributor(
            address(kwenta),
            address(stakingRewardsV2),
            address(rewardEscrowV2),
        ...
        goForward(1 weeks);
        goForward(time);
        uint proportionalFees1 = (((amount * 1 weeks) / (time + 1 weeks)) * staking1 / (staking1 + staking2 + staking3));
        vm.expectEmit(true, true, true, true);
        ...
        uint proportionalFees2 = (((amount * 1 weeks) / (time + 1 weeks)) * staking2 / (staking1 + staking2 + staking3));
        vm.expectEmit(true, true, true, true);
        emit EpochClaim(address(user2), 1, proportionalFees2);
        tokenDistributorOffset.claimEpoch(address(user2), 1);
        
        uint proportionalFees3 = (((amount * 1 weeks) / (time + 1 weeks)) * staking3 / (staking1 + staking2 + staking3));
        vm.expectEmit(true, true, true, true);
        emit EpochClaim(address(user3), 1, proportionalFees3);
        tokenDistributorOffset.claimEpoch(address(user3), 1);
