...
contract TokenDistributor is ITokenDistributor {
    ...
    uint256 public constant MAX_OFFSET_DAYS = 6;
    ...
    constructor(address _rewardsToken, address _stakingRewardsV2, uint256 _daysToOffsetBy) {
        if (_rewardsToken == address(0) || _stakingRewardsV2 == address(0)) {
            revert ZeroAddress();
        }
        ...
        uint256 startOfThisWeek = _startOfWeek(block.timestamp);
    ...
    function checkpointToken() public override {
        ...
        uint256 toDistribute = tokenBalance - lastTokenBalance;
        lastTokenBalance = tokenBalance;

        uint256 previousCheckpoint = lastCheckpoint;
        uint256 sinceLast = block.timestamp - previousCheckpoint;
        lastCheckpoint = block.timestamp;
        /// @dev this/nextWeek is for iterating through time
        uint256 thisWeek = _startOfWeek(previousCheckpoint);
        uint256 nextWeek = 0;

        /// @dev Loop for potential missed weeks
        /// iterates until caught up, unlikely to go to 52 weeks
        for (uint256 i = 0; i < WEEKS_IN_YEAR;) {
            nextWeek = thisWeek + 1 weeks;
            uint256 thisEpoch = _epochFromTimestamp(thisWeek);
            if (block.timestamp < nextWeek) {
                ...
                if (sinceLast == 0) {
                    ...
                    tokensPerEpoch[thisEpoch] +=
                        (toDistribute * (block.timestamp - previousCheckpoint)) / sinceLast;
                }
                break;
            } else {
                ...
                tokensPerEpoch[thisEpoch] +=
                    (toDistribute * (nextWeek - previousCheckpoint)) / sinceLast;
            }
    ...
    /// @inheritdoc ITokenDistributor
    function claimEpoch(address to, uint256 epochNumber) public override {
    ...
    function _claimEpoch(address to, uint256 epochNumber)
        internal
        returns (uint256 proportionalFees)
    {
    ...
    /// @inheritdoc ITokenDistributor
    function claimMany(address to, uint256[] calldata epochs) public {
        _checkpointWhenReady();
        uint256 length = epochs.length;
        uint256 totalProportionalFees;
        for (uint256 i; i < length;) {
            uint256 epochNumber = epochs[i];
    ...
    /// @inheritdoc ITokenDistributor
    function calculateEpochFees(address to, uint256 epochNumber)
        public
        view
        override
        returns (uint256)
    {
        uint256 epochStart = _startOfEpoch(epochNumber);
    ...
    /// @inheritdoc ITokenDistributor
    function claimedEpoch(address to, uint256 epochNumber) public view override returns (bool) {
        return _claimedEpochsBitMap[to].get(epochNumber);
    }

    /// @notice function for calculating the start of a week with an offset
    function _startOfWeek(uint256 timestamp) internal view returns (uint256) {
    ...
    function _startOfEpoch(uint256 epochNumber) internal view returns (uint256) {
    ...
    function _isEpochReady(uint256 epochNumber) internal view {
    ...
    function _epochFromTimestamp(uint256 timestamp) internal view returns (uint256) {
